...
# -*- coding: utf-8 -*-
"""
개선된 패널 감지 시스템 - 정확한 사각형 감지
Improved Panel Detection System - Accurate Rectangle Detection
"""
...
class ImprovedPanelDetector:
    ...
    def __init__(self):
        ...
        self.panel_rectangle = None  # 정확한 사각형 좌표
    ...
    def setup_ui(self):
        """사용자 인터페이스 설정"""
        self.root = tk.Tk()
        self.root.title("개선된 패널 검사 시스템 - 정확한 사각형 감지")
        ...
        ttk.Button(inspect_frame, text="사각형 보정", 
                  command=self.correct_rectangle).pack(side=tk.LEFT, padx=2)
        ...
        # 초기 로그
        self.log_result("개선된 패널 검사 시스템이 시작되었습니다.")
        self.log_result("1. 카메라를 시작하세요")
        self.log_result("2. 패널 감지 후 사각형 보정을 사용하세요")
        self.log_result("3. 정확한 사각형으로 검사를 시작하세요")
    ...
    def draw_panel_detection(self, frame):
        ...
        if self.panel_contour is not None:
            cv2.drawContours(display_frame, [self.panel_contour], -1, (128, 128, 128), 2)
        ...
        if self.panel_rectangle is not None:
            ...
            # 모서리 마커
            corner_size = 15
            cv2.line(display_frame, (x, y), (x + corner_size, y), (0, 255, 0), 5)
            cv2.line(display_frame, (x, y), (x, y + corner_size), (0, 255, 0), 5)
            cv2.line(display_frame, (x+w, y), (x+w - corner_size, y), (0, 255, 0), 5)
            cv2.line(display_frame, (x+w, y), (x+w, y + corner_size), (0, 255, 0), 5)
            cv2.line(display_frame, (x, y+h), (x + corner_size, y+h), (0, 255, 0), 5)
            cv2.line(display_frame, (x, y+h), (x, y+h - corner_size), (0, 255, 0), 5)
            cv2.line(display_frame, (x+w, y+h), (x+w - corner_size, y+h), (0, 255, 0), 5)
            cv2.line(display_frame, (x+w, y+h), (x+w, y+h - corner_size), (0, 255, 0), 5)
        ...
        try:
            ...
            if panel_contour is not None:
                ...
                if rectangle is not None:
                    ...
                    self.log_result("사각형 보정을 사용하여 더 정확하게 조정할 수 있습니다.")
                else:
    ...
    def extract_rectangle_from_contour(self, contour):
        ...
        try:
            ...
            # 2. 윤곽선 근사화 (더 정확한)
            epsilon = 0.01 * cv2.arcLength(contour, True)  # 더 작은 epsilon
    ...
    def correct_rectangle(self):
        ...
        try:
            # 더 정확한 사각형 추출
            corrected_rectangle = self.extract_precise_rectangle(self.panel_contour)
            
            if corrected_rectangle is not None:
                self.panel_rectangle = corrected_rectangle
                self.update_panel_info()
                self.log_result("사각형이 보정되었습니다.")
            else:
                self.log_result("사각형 보정에 실패했습니다.")
                
        except Exception as e:
            self.log_result(f"사각형 보정 오류: {str(e)}")
    
    def extract_precise_rectangle(self, contour):
        """정밀한 사각형 추출"""
        try:
            # 1. 윤곽선의 경계 상자
            x, y, w, h = cv2.boundingRect(contour)
            ...
            # 2. 윤곽선을 더 정확하게 근사화
            epsilon = 0.005 * cv2.arcLength(contour, True)  # 매우 작은 epsilon
            approx = cv2.approxPolyDP(contour, epsilon, True)
            
            # 3. 사각형의 꼭짓점 찾기
            if len(approx) >= 4:
                # 꼭짓점들을 정렬 (좌상, 우상, 우하, 좌하)
                approx = approx.reshape(-1, 2)
                
                # x+y가 가장 작은 점 (좌상)
                top_left = approx[np.argmin(approx.sum(axis=1))]
                # x-y가 가장 작은 점 (우상)
                top_right = approx[np.argmin(approx[:, 0] - approx[:, 1])]
                # x+y가 가장 큰 점 (우하)
                bottom_right = approx[np.argmax(approx.sum(axis=1))]
                # y-x가 가장 큰 점 (좌하)
                bottom_left = approx[np.argmax(approx[:, 1] - approx[:, 0])]
    ...
    def update_panel_info(self):
        ...
        try:
            ...
            # 패널 정보 텍스트
            info_text = f"패널 정보 (정확한 사각형):\n"
            info_text += f"위치: ({x}, {y})\n"
            info_text += f"크기: {w} x {h} 픽셀\n"
            info_text += f"면적: {area:,} 픽셀²\n"
            info_text += f"종횡비: {aspect_ratio:.3f}\n"
            
            # 디스플레이 타입 추정
            if 1.75 <= aspect_ratio <= 1.78:
                info_text += f"추정 타입: 16:9 디스플레이 (정확)\n"
            elif 1.58 <= aspect_ratio <= 1.62:
                info_text += f"추정 타입: 16:10 디스플레이 (정확)\n"
            elif 1.32 <= aspect_ratio <= 1.35:
                info_text += f"추정 타입: 4:3 디스플레이 (정확)\n"
            elif 2.35 <= aspect_ratio <= 2.40:
                info_text += f"추정 타입: 21:9 디스플레이 (정확)\n"
            elif 0.99 <= aspect_ratio <= 1.01:
                info_text += f"추정 타입: 1:1 디스플레이 (정확)\n"
            else:
                info_text += f"추정 타입: 사용자 정의 ({aspect_ratio:.3f})\n"
            
            # 정확도 정보
            if self.panel_contour is not None:
                contour_area = cv2.contourArea(self.panel_contour)
                accuracy = (area / contour_area) * 100 if contour_area > 0 else 0
                info_text += f"사각형 정확도: {accuracy:.1f}%\n"
    ...
    def start_inspection(self):
        """검사 시작"""
        if self.panel_rectangle is None:
            messagebox.showwarning("경고", "먼저 패널을 감지하고 사각형을 보정하세요.")
            return
